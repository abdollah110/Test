...
void fake_Apply_PF() {
    ...
    //*****************************************************************************************
    //*****************************************************************************************
    const char * array[] = {"FakeRate_MMTT_apply_pf", "FakeRate_EETT_apply_pf"};
    for (int g = 0; g < 2; g++) {
        TH1D *Pt_Met_Sig = (TH1D*) Wtnu->Get(array[g]);
        double estimate = 0;

        for (int i = 10; i < Pt_Met_Sig->GetNbinsX(); i++) {

            double Value = Pt_Met_Sig->GetBinContent(i + 1);
    ...
    //*****************************************************************************************
    //*****************************************************************************************
    const char * array[] = {"FakeRate_MMMT_apply_pf", "FakeRate_EEMT_apply_pf"};
    for (int g = 0; g < 2; g++) {

         if (Wtnu->Get(array[g])){
        TH1D *Pt_Met_Sig = (TH1D*) Wtnu->Get(array[g]);

        double FrateX = fake_muon[1];
        double estimate = Pt_Met_Sig->GetEntries() * (FrateX) / (1 - FrateX);
        ...
        result_FF_Err[g+2] = 0;
        result_FF_Num[g+2] = 0;
          cout << "Estimate " << array[g] << "   " << 0 << "  +-   " << 0 << " (" << 0 << ") " << endl;
         }
    }
    //*****************************************************************************************
    //*****************************************************************************************
    const char * array[] = {"FakeRate_MMET_apply_pf", "FakeRate_EEET_apply_pf"};
    for (int g = 0; g < 2; g++) {

        if (Wtnu->Get(array[g])){
        TH1D *Pt_Met_Sig = (TH1D*) Wtnu->Get(array[g]);

        double FrateX = fake_electron[1];
        double estimate = Pt_Met_Sig->GetEntries() * (FrateX) / (1 - FrateX);
    ...
    //*****************************************************************************************
    //*****************************************************************************************
    const char * array[] = {"FakeRate_MMME_apply_pf", "FakeRate_EEEM_apply_pf"};
    for (int g = 0; g < 2; g++) {

        if (Wtnu->Get(array[g])){
        TH1D *Pt_Met_Sig = (TH1D*) Wtnu->Get(array[g]);

        double FrateX = fake_muon[0];
        double estimate = Pt_Met_Sig->GetEntries() * (FrateX) / (1 - FrateX);
    ...
    TFile * F_Result_FF = new TFile("Result_PF.root", "RECREATE");
    F_Result_FF->cd();
