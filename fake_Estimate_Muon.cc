...
void fake_Estimate_Muon() {

    TFile *data_Ele_data = new TFile("Data.root");


//    TH1D *Num_data_loose = (TH1D*) data_Ele_data->Get("FakeRate_Muon_Loose_After_Jet");
//    TH1D *Num_data_tight = (TH1D*) data_Ele_data->Get("FakeRate_Muon_Tight_After_Jet");
//    TH1D *Denum_data = (TH1D*) data_Ele_data->Get("FakeRate_Muon_Before_Jet");


    
    TH1D *Num_data_loose = (TH1D*) data_Ele_data->Get("FakeRate_MT_Muon_Loose_After_0");
    TH1D *Num_data_tight = (TH1D*) data_Ele_data->Get("FakeRate_MT_Muon_Tight_After_0");
    TH1D *Denum_data = (TH1D*) data_Ele_data->Get("FakeRate_MT_Muon_Pt_Before_0");
    ...
    TLatex * tex = new TLatex(50, 0.6, "Loose Muon fake rate ");
   ...
   //Text in Histogram
    ...
    MyC->SaveAs("Fake_estimate_Muon.png");
    MyC->SaveAs("Fake_estimate_Muon.pdf");




    TFile *varPar = new TFile("Fake_Muon.root", "RECREATE");
    varPar->cd();

    TH1F * FitVar_FitError = new TH1F("fakeRate_Muon", "fakeRate_Muon", 6, 0, 6);
    ...
    cout<<"**************************************************************************"<<endl;
    cout<< "Muon loose FR=  " << Num_rebin_data_loose->GetEntries()<< "/"<< DeNum_rebin_data->GetEntries()<<  "  =  "<<Num_rebin_data_loose->GetEntries()*1.0 / DeNum_rebin_data->GetEntries()<<endl;
    cout<< "Muon tight FR=  " << Num_rebin_data_tight->GetEntries()<< "/"<< DeNum_rebin_data->GetEntries()<<"  =  " << Num_rebin_data_tight->GetEntries()*1.0 / DeNum_rebin_data->GetEntries()<<endl;
    cout<<"**************************************************************************"<<endl;
    ...
    TFile* fOUT = new TFile("Fake_estimate_Muon.root", "RECREATE");
    fOUT->cd();
